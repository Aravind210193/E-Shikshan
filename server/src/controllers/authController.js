...
const addCertificate = async (req, res) => {
  try {
    ...
    user.certificatesAndBadges.push(newCertificate);
    await user.save();

    res.status(201).json({
      success: true,
      message: 'Certificate added successfully',
      certificate: user.certificatesAndBadges[user.certificatesAndBadges.length - 1]
    });
  } catch (error) {
...
const getCertificates = async (req, res) => {
  try {
    const user = await User.findById(req.user._id)
      .select('certificatesAndBadges')
      .populate('certificatesAndBadges.courseId', 'title');
    ...
    res.json({
      success: true,
      certificates: user.certificatesAndBadges || []
    });
  } catch (error) {
...
const updateCertificate = async (req, res) => {
  try {
    ...
    const certificate = user.certificatesAndBadges.id(id);
...
const deleteCertificate = async (req, res) => {
  try {
    ...
    // Only allow deletion of manually added certificates
