...
const AdminResumes = () => {
  ...
  const openEdit = (item) => {
    ...
    setFormData({
      ...
      subTemplates: item.subTemplates || [],
              ...
              <div className="p-5">
                {modalMode === 'view' ? (
                  <div className="space-y-3 text-gray-300">
                    ...
                    <div><span className="text-gray-400 text-sm">Color</span><p className="text-white">{selected?.color}</p></div>
                    <div><span className="text-gray-400 text-sm">Preview Icon</span><p className="text-white">{selected?.preview}</p></div>
                    ...
                    {/* Sub Templates */}
                    <div>
                      <span className="text-gray-400 text-sm">Sub-Templates</span>
                      <ol className="list-decimal ml-5 text-white space-y-2">
                        {(selected?.subTemplates || []).map((s, i) => (
                          <li key={i}>
                            <p><span className="font-bold text-emerald-400">{s.name}</span> <span className="text-gray-500">({s.id})</span></p>
                            <p className="text-sm italic text-gray-400">{s.description}</p>
                            <p className="text-xs text-gray-500">Features: {(s.features || []).join(', ')}</p>
                          </li>
                        ))}
                      </ol>
                    </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input value={formData.color} onChange={(e) => setFormData({ ...formData, color: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white" />
                      </div>
                      <div>
                        <label className="text-sm text-gray-400">Preview Icon (Emoji)</label>
                        <input value={formData.preview} onChange={(e) => setFormData({ ...formData, preview: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white" />
                      </div>
                      <div>
                        <label className="text-sm text-gray-400">Recommended</label>
                        <select value={formData.recommended ? 'true' : 'false'} onChange={(e) => setFormData({ ...formData, recommended: e.target.value === 'true' })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white">
                          <option value="true">Yes</option>
                          <option value="false">No</option>
                        </select>
                      </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={() => setFormData({ ...formData, subTemplates: [...(formData.subTemplates || []), { id: `sub-${Date.now()}`, name: '', description: '', colorScheme: 'blue', features: [] }] })} className="px-3 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-white">Add Sub-Template</button>
                      </div>
                        ...
                        <div key={i} className="grid grid-cols-1 md:grid-cols-12 gap-3 bg-gray-800/60 border border-gray-700 rounded-xl p-3">
                          ...
                          <div className="md:col-span-1 flex items-center justify-end">
                            <button type="button" onClick={() => {
                              ...
                              setFormData({ ...formData, subTemplates: newSubs });
                            }} className="p-2 hover:bg-gray-700 rounded-lg"><Trash2 className="w-4 h-4 text-red-400" /></button>
                          </div>
